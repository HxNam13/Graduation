/* ============================================
   GRADUATION — Nguyễn Hữu Nam
   Minimalist · MU Red · Grain
   ============================================ */
...
function tick() {
    ...
    if (d <= 0) {
        const cd = $('.countdown');
        if (cd) cd.innerHTML = '<p style="font-family:var(--display);font-size:1rem;font-weight:700;color:var(--red);letter-spacing:.1em;text-transform:uppercase">🎓 Buổi lễ đã diễn ra!</p>';
    }
}
tick();
...
(function initRSVP() {
    ...
    form.addEventListener('submit', async e => {
        ...
        if (!att) { toast('Chọn xác nhận'); return; }

        btn.classList.add('loading');
        btn.disabled = true;

        try {
            const data = { name, attendance: att.value, timestamp: new Date().toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' }) };
            ...
            setTimeout(() => {
                ...
                const msg = $('#res-msg');
                if (msg) msg.textContent = att.value === 'Tham dự' ? 'Hẹn gặp bạn tại buổi lễ! 🎉' : 'Cảm ơn bạn đã phản hồi.';
